...
使用异步 API 的脚本具有不同的 `main` 功能。 它是一 `async` 个将 作为第 `Excel.RequestContext` 一个参数的函数。
...
因为脚本和工作簿在不同的位置运行，所以两者之间的任何数据传输都需要时间。 在异步 API 中，命令将排入队列，直到脚本显式调用 `sync` 操作来同步脚本和工作簿。 脚本可以独立运行，直到需要执行以下任一操作：
...
每当异步脚本需要读取工作簿数据或将数据写入工作簿时，请调用 方法 `RequestContext.sync` ，如下所示：
...
异步脚本必须先从工作簿加载数据，然后才能读取数据。 但是，从整个工作簿加载数据将大大降低脚本的速度。 `load`方法使脚本可以专门说明应从工作簿中检索哪些数据。
...
从工作簿返回信息的异步 API 中的方法具有与范例类似的 `load` / `sync` 模式。 举个例子，`TableCollection.getCount`获取集合中的表的数量。 `getCount` 返回一`ClientResult<number>`，这意味着返回的`value`中的 [`ClientResult`](/javascript/api/office/officeextension.clientresult?view=excel-js-online&preserve-view=true) 属性是一个数字。 在调用 `context.sync()` 之前，脚本无法访问此值。 与加载属性很相似，直到 `sync` 调用，`value` 是本地 "空" 值。
...
当前 API 模型不使用 `load` 、 `sync` 或 `RequestContext` 。 这使得脚本更易于编写和维护。 转换旧脚本的最佳资源是 Stack [Overflow](https://stackoverflow.com/questions/tagged/office-scripts)。 在那里，你可以向社区请求特定方案的帮助。 以下指南应有助于概述需要执行一般步骤。

1. 创建新脚本，将旧的异步代码复制到该脚本中。 请务必不要包含旧 `main` 方法签名，而使用当前 `function main(workbook: ExcelScript.Workbook)` 方法签名。

2. 删除所有 `load` 和 `sync` 调用。 它们不再需要。

3. 所有属性都已删除。 现在，您通过 和 方法访问这些对象，因此您需要将那些属性引用切换到 `get` `set` 方法调用。 例如，现在将使用以下类似方法，而不是通过属性访问设置单元格的填充 `mySheet.getRange("A2:C2").format.fill.color = "blue";` 颜色： `mySheet.getRange("A2:C2").getFormat().getFill().setColor("blue");`
