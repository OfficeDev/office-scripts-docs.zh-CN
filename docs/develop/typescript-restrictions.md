...
ms.locfileid: "60924101"
---
# <a name="typescript-restrictions-in-office-scripts"></a>脚本中的 TypeScript Office
...
在[](https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes.html) TypeScript 中，写入类型是可选的，因为可以推断出这些类型。 但是Office脚本要求变量不能为任何[类型](https://www.typescriptlang.org/docs/handbook/basic-types.html#any)。 在脚本中 `any` 不允许显式和隐式Office脚本。 这些情况报告为错误。
...
在上一个屏幕截图 `[2, 14] Explicit Any is not allowed` 中，指示行 #2，列 #14 定义 `any` 类型。 这可以帮助您找到错误。
...
## <a name="constructors-dont-support-office-scripts-apis-and-console-statements"></a>构造函数不支持Office脚本 API 和 `console` 语句

`console`语句和许多Office脚本 API 需要与工作簿Excel同步。 这些同步使用 `await` 脚本的已编译运行时版本中的语句。 `await` 在构造函数 [中不受支持](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Classes/constructor)。 如果你需要具有构造函数的类，请避免Office脚本 API 或 `console` 这些代码块中的语句。
